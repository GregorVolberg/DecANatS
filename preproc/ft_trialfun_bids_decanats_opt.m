...
target_corners    = unique(events.label_abs_pos(startsWith(events.label_opt_pos_abs, 'corner_')));
on_off_corners    = cellfun(@(x) (find(diff(ismember(events.label_opt_pos_abs, x))) + 1), target_corners, ...
                    'UniformOutput', false);
