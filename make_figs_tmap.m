...
lower_t = tinv(1-0.05/2, 9);
upper_t = tinv(1-0.001/2, 9);
fprintf('\n!!!!!!!!!!!!!! ATTENTION !!!!!!!!!!!!\n');
fprintf('Used fixed df = 9 for scaling of t map\n');
...
for i = 1:numFiles
    ...
    if ~isempty(hAx)
        ...
        elseif mod(i,2) == 0
        title(targetAx, [tile_labels{i}, ': t-value']);
        targetAx.CLim = [lower_t upper_t];
        cb.Ticks = [2.5:1:4.5];        
...
savefig(newFig, ['./md_images/', erase(allFilesLabels{k}, whitespacePattern), '_t.fig']);
exportgraphics(newFig, ['./md_images/', erase(allFilesLabels{k}, whitespacePattern), '_t.png'], 'Resolution', 300);
close(newFig);
end
